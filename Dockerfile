...
# Laravel 8.32 predates PHP 8.2. Keep its exact lockfile, but prevent its own
# bootstrap from re-enabling PHP 8.2 deprecation notices as fatal exceptions.
# Runtime validation is intentionally handled by entrypoint.sh so build failures
# and application bootstrap failures remain distinguishable in Railway.
