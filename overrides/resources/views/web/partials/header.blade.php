...
<header class="header-area header-sticky">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <nav class="main-nav">
                    <a href="{{route('welcome')}}" class="logo yellow-duck-lockup" aria-label="البطة الصفرا لخدمات السوشيال ميديا">
                    ...
                    <ul class="nav">
                        <li><a class="link" href="@if($links) {{route('welcome').'#welcome'}}@else{{'#welcome'}}@endif">{{Helper::getLang('Home')}}</a></li>
                        ...
                        <li>
                            <div class="dropdown">
                                <div class="dropdown-toggle pt-2" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    <img width="30" src="{{asset('images/'.$lang->image)}}" alt="{{$lang->name}}">
                                    <i class="fas fa-chevron-down"></i>
                                </div>
                                <div class="dropdown-menu">
                                    @foreach ($languages as $language)
                                        <a class="dropdown-item" href="{{route('languages.set-language',$language->id)}}">
                                            <img width="20" src="{{asset('images/'.$language->image)}}" alt="{{$language->name}}">
                                            {{$language->name}}
                                        </a>
                                    @endforeach
                                </div>
