...
class PaymentController extends Controller
{
    ...
    public function payWithManualDeposit(Request $request)
    {
        ...
        $proofBytes = file_get_contents($proof->getRealPath());
        if ($proofBytes === false) {
            return back()->withErrors(['proof' => 'تعذر قراءة صورة إثبات التحويل. حاول رفعها مرة أخرى.'])->withInput();
        }

        $this->ensureDepositProofTable();

        DB::transaction(function () use ($method, $amount, $fee, $credit, $rate, $request, $destination, $proof, $proofBytes) {
    ...
    public function payWithStrip($request)
    {
        ...
        if (!$customer->stripe_token) {
            $result = \Stripe\Customer::create(['email' => $customer->email, 'source' => $stripe_token['id']]);
            if ($result && $result->id) {
        ...
        if ($customer->stripe_token) {
            $result = \Stripe\Charge::create(['currency' => 'usd', 'customer' => $customer->stripe_id, 'amount' => $amount_total]);
    ...
    private function ensureManualPaymentMethods()
    {
        ...
        $defaults = [
            [
                'name' => 'Vodafone Cash', 'min' => 55, 'max' => 550000, 'fee' => 0,
                'environment' => 'production', 'api_key' => null, 'private_key' => '01205323440',
                'client_id' => 'حول على رقم فودافون كاش ثم اكتب رقم الهاتف والمبلغ في النموذج.', 'image' => 'vodafone-cash.svg',
            ],
            [
                'name' => 'InstaPay Egypt', 'min' => 55, 'max' => 550000, 'fee' => 0,
                'environment' => 'production', 'api_key' => null, 'private_key' => 'menna_206@instapay',
                'client_id' => 'استخدم QR أو حوّل إلى menna_206@instapay ثم اكتب بيانات التحويل وارفق صورة الإثبات.', 'image' => 'instapay.svg',
            ],
        ];
